...
grep -q 'api.signInEmail' "$ROUTE" || { echo "FAIL: signup does not verify the submitted password"; exit 1; }
grep -q 'Signup credential verification failed' "$ROUTE" || { echo "FAIL: signup credential verification diagnostics missing"; exit 1; }
